        ...
        <div className="grid lg:grid-cols-[0.85fr_1.15fr] gap-12 lg:gap-20 items-center">
          ...
          >
            <div className="flex items-center gap-2 text-[#C084FC] mb-5">
              <Sparkles size={16} />
              <span className="font-mono text-xs uppercase tracking-[0.18em]">Currently in development</span>
            </div>
            <h2 className="font-display font-black text-5xl sm:text-6xl md:text-7xl leading-[0.9] tracking-tight text-white">
              Revision<span className="text-[#A78BFA]">.</span>TN
            </h2>
            <p className="font-display text-2xl md:text-3xl text-[#E9D5FF] mt-7 leading-tight">
              A new way to learn, revise, and grow.
            </p>
            <p className="max-w-md text-[#A1A1AA] leading-relaxed mt-6">
              A Tunisian education project I am actively building for students, young people, teachers, and parents.
            </p>
            <div className="inline-flex items-center gap-2 mt-8 text-sm text-[#D8B4FE] group cursor-default">
    ...
    >
      ...
      <div className="relative w-full max-w-[470px] rounded-[1.75rem] border border-[#A78BFA]/25 bg-[#160E24]/90 p-4 shadow-[0_24px_80px_rgba(76,29,149,0.32)] backdrop-blur-xl sm:p-5">
        <div className="flex items-center justify-between border-b border-white/10 px-2 pb-4">
          ...
          <span className="font-mono text-[10px] uppercase tracking-widest text-[#A78BFA]">Your next step</span>
        </div>
        ...
        <div className="relative mt-5 overflow-hidden rounded-2xl border border-white/10 bg-[#0E0916] p-5 sm:p-6">
          ...
          <div className="relative flex items-start justify-between gap-4">
            <div>
              <p className="font-mono text-[10px] uppercase tracking-widest text-[#A78BFA]">Learning roadmap</p>
              <p className="font-display text-xl font-bold text-white mt-2">Build your momentum.</p>
            </div>
            <GraduationCap size={26} className="text-[#D8B4FE]" />
          </div>
          <div className="grid grid-cols-3 gap-2 mt-8">
            {["Discover", "Learn", "Grow"].map((label, index) => (
              <div key={label} className="rounded-xl border border-white/8 bg-white/[0.035] p-3">
                ...
                <p className="text-[11px] text-[#C4B5FD]">{label}</p>
              </div>
            ))}
          </div>
          <div className="flex items-center gap-2 mt-5 text-xs text-[#71717A]"><BookOpen size={13} className="text-[#A78BFA]" /> A space for better study days</div>
        </div>
        <div className="flex items-center justify-between px-2 pt-4 text-[10px] font-mono uppercase tracking-widest text-[#6D5A85]"><span>For the Tunisian learning community</span><span>01 / 03</span></div>
      </div>
    </motion.div>
  );
}